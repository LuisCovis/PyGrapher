...
def setup(
    ...
    ax.xaxis.set_minor_locator(ticker.MultipleLocator(0.1))#(ax.get_xlim()[1] - ax.get_xlim()[0]) / (5 * len(ax.xaxis.get_major_ticks()))))
    ax.yaxis.set_minor_locator(ticker.MultipleLocator(0.1))#(ax.get_ylim()[1] - ax.get_ylim()[0]) / (5 * len(ax.yaxis.get_major_ticks()))))
    ax.tick_params(axis='x', which="both", colors=cfg.color["text_color"])    
    ax.tick_params(axis='y', which="both", colors=cfg.color["text_color"]) 
    ax.set_yticks(np.arange(_ylim[0], _ylim[1]+0.1, 0.5))
